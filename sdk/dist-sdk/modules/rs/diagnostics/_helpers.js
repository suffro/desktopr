...
const diagnosticsSettings = async (core, settings) => {
    ...
    if (settings?.retentionDaysLogs && !utils_1.Num.isU32(settings.retentionDaysLogs))
        throw ("[retentionDaysLogs] the value must be a U32 integer number");
    if (settings?.retentionDaysCrashes && !utils_1.Num.isU32(settings.retentionDaysCrashes))
