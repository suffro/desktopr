...
pub fn dtr_app_info(app: AppHandle) -> Result<AppInfo, String> {
    ...
    let exec_dir = exec_path.as_ref().and_then(|p| {
        std::path::Path::new(p)
            .parent()
    ...
    for (label, win) in app.webview_windows() {
        let size: PhysicalSize<u32> = win.inner_size().unwrap_or(PhysicalSize {
            width: 0,
            height: 0,
        });
        let pos: PhysicalPosition<i32> = win
            .outer_position()
