...
fn apply_menu_config(app: &AppHandle<Wry>, cfg: MenuConfig) -> tauri::Result<()> {
    ...
    let refs: Vec<&dyn tauri::menu::IsMenuItem<Wry>> = subs
        .iter()
    ...
    app.manage(CheckState {
        map: Mutex::new(check_map),
    });
    app.manage(CxCheckState {
...
fn init_menu_from_json_internal(
    app: &AppHandle<Wry>,
    cfg_json: &serde_json::Value,
) -> tauri::Result<()> {
    let cfg: MenuConfig =
        serde_json::from_value(cfg_json.clone()).map_err(|e| -> tauri::Error {
            anyhow::anyhow!(
...
#[tauri::command]
pub fn dtr_init_menu_from_json(
    app: AppHandle<Wry>,
    cfg_json: serde_json::Value,
) -> Result<(), String> {
    ...
    for it in items {
        match it {
            MenuItemUnion::Custom(MenuConfigCustomItem {
                ...
                if matches!(
                    interaction,
                    crate::helpers::menu_builder::MenuInteraction::Check
                ) {
...
fn predefined_ids_and_label(
    ...
    match item {
        MenuPredefinedMenuItemSlug::About => {
            ("predefined.about".to_string(), "About".to_string(), None)
        }
        ...
        MenuPredefinedMenuItemSlug::Hide => {
            ("predefined.hide".to_string(), "Hide".to_string(), None)
        }
        ...
        MenuPredefinedMenuItemSlug::Separator => {
            ("predefined.separator".to_string(), "".to_string(), None)
        }
    }
}
...
fn build_submenu_from_section_handle(
    ...
    for item in &sec.items {
        match item {
            ...
            MenuItemUnion::Predefined(MenuConfigPredefinedItem {
                item, custom_label, ..
            }) => {
                    ...
                    match subitem {
                        ...
                        MenuItemUnion::Custom(MenuConfigCustomItem {
                            ...
                            MenuInteraction::Click => {
                                ...
                                nested_builder =
                                    nested_builder.item(nested_menu_items.last().unwrap());
                            }
                            MenuInteraction::Check => {
                                let mut b =
                                    CheckMenuItemBuilder::with_id(id, label).enabled(*enabled);
                                b = b.checked(checked.unwrap_or(false));
                                let ci = b.build(app)?;
                                nested_check_items.push(ci);
                                nested_builder =
                                    nested_builder.item(nested_check_items.last().unwrap());
                            }
                        },
                        MenuItemUnion::Predefined(MenuConfigPredefinedItem {
                            item,
...
fn is_current_platform_in(list: &[MenuPlatform]) -> bool {
    #[cfg(target_os = "macos")]
    {
        list.contains(&MenuPlatform::Macos)
    }
    #[cfg(target_os = "windows")]
    {
        list.contains(&MenuPlatform::Windows)
    }
    #[cfg(target_os = "linux")]
    {
        list.contains(&MenuPlatform::Linux)
    }
}
    ...
    app.on_menu_event(move |app, ev| {
        ...
        let _ = app.emit(
            ...
            MenuEventPayload {
                ...
                parent_label,
                parent_id,
                last_focused_window_label: Some(window_label),
            },
        );
    });
}
