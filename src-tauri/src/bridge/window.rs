...
pub async fn dtr_win_open(
    ...
    if !s.is_empty() {
        let webview_url = WebviewUrl::External(s.parse::<Url>().map_err(|e| e.to_string())?);
...
pub fn dtr_win_get_info(
    ...
    if let Some(win) = target {
        ...
        let inner_position = win
            .inner_position()
            .ok()
            .map(|p| WindowPositionInfo { x: p.x, y: p.y });
        let outer_position = win
            .outer_position()
            .ok()
            .map(|p| WindowPositionInfo { x: p.x, y: p.y });
