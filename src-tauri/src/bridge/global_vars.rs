...
impl EnvState {
    ...
    pub fn init<R: Runtime>(app: &AppHandle<R>) -> tauri::Result<Self> {
        ...
        let path = app.path().resolve("env.json", BaseDirectory::AppConfig)?;
    ...
    fn save(&self, store: &EnvStore) -> Result<(), String> {
        ...
        fs::write(&self.path, json).map_err(|e| format!("Failed to write env store file: {e}"))
    }
...
pub fn dtr_global_vars_get(
    ...
    state.with(|store| Ok(store.vars.get(&key).cloned()))
}
...
#[tauri::command]
pub fn dtr_global_vars_remove(key: String, state: State<'_, EnvState>) -> Result<bool, String> {
    state.with_mut(|store| Ok(store.vars.remove(&key).is_some()))
}

#[tauri::command]
pub fn dtr_global_vars_list(state: State<'_, EnvState>) -> Result<HashMap<String, String>, String> {
    state.with(|store| {
