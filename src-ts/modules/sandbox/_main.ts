...
export function buildSandbox(core: { invoke: BubbledeskAPI["invoke"] }): SandboxInterface {
    return {
      call: (input: string): Promise<string> => core.invoke("bd_sandbox_call", {input}),
      modules: {
        ...
        addFromBytes: (name: string, contents: U8[]): Promise<boolean> => core.invoke("bd_sandbox_save_module", {name, contents}),
        add: (name: string, maxBytes?: U64): Promise<boolean> => core.invoke("bd_sandbox_pick_and_save_module", {maxBytes, defaultName: name}),
      },
