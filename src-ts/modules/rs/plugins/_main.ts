...
export function buildPlugins(core: { invoke: DesktoprAPI["invoke"] }): PluginsInterface {
    return {
      ...
      addFromBytes: (name: string, contents: U8[]): Promise<any> => core.invoke("dtr_worker_add_module", {name: normalizeModuleName(name), contents}),
      add: (name: string, maxBytes?: U64): Promise<PluginAddResult> => core.invoke("dtr_worker_pick_and_add_module", {maxBytes, defaultName: normalizeModuleName(name)}),
