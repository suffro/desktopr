...
function scope(
  ...
  const ensureDataNotIsolated = () => {
    ...
    if (label && label.trim().length > 0) {
      throw new Error(
        "Persistent data operations are not available in isolated companion windows. Use the cache scope (Bubbledesk.fs.cache) for per-session storage."
      );
    }
  };
  ...
  return {
    listContent: (rel: string = "") => {
      ...
      return core.invoke<FsEntry[]>("bd_fs_list_dir", {
        rel,
        permanent,
    ...
    newDirectory: (rel: string) => {
      ensureDataNotIsolated();
      return core.invoke<void>("bd_fs_mkdir", {
        rel,
        permanent,
        windowLabel: getWindowLabelIfCompanion(),
      });
    ...
    remove: (rel: string, recursive = false) => {
      ensureDataNotIsolated();
      return core.invoke<void>("bd_fs_rm", {
        rel,
        recursive,
        permanent,
        windowLabel: getWindowLabelIfCompanion(),
    ...
    stat: (rel: string = "") => {
      ensureDataNotIsolated();
      return core.invoke<FsEntry>("bd_fs_stat", {
        rel,
        permanent,
        windowLabel: getWindowLabelIfCompanion(),
      });
    ...
    writeText: (rel, contents, opts) => {
      ensureDataNotIsolated();
      return core.invoke<void>("bd_fs_write_text", {
        rel,
        permanent,
        contents,
    ...
    readText: (rel) => {
      ensureDataNotIsolated();
      return core.invoke<string>("bd_fs_read_text", {
        rel,
        permanent,
        windowLabel: getWindowLabelIfCompanion(),
      });
    ...
    writeBytes: (rel, base64, opts) => {
      ensureDataNotIsolated();
      return core.invoke<void>("bd_fs_write_bytes", {
        rel,
        permanent,
        dataBase64: base64,
        createDirs: opts?.createDirs,
    ...
    readBytes: (rel) => {
      ensureDataNotIsolated();
      return core.invoke<string>("bd_fs_read_bytes", {
        rel,
        permanent,
        windowLabel: getWindowLabelIfCompanion(),
      });
    ...
    exists: (rel) => {
      ensureDataNotIsolated();
      return core.invoke<boolean>("bd_fs_exists", {
        rel,
        permanent,
        windowLabel: getWindowLabelIfCompanion(),
      });
    ...
    move: (src, dest, opts) => {
      ensureDataNotIsolated();
      return core.invoke<void>("bd_fs_move", {
        src,
        dest,
        permanent,
        createDirs: opts?.createDirs,
    ...
    copy: (src, dest, opts) => {
      ensureDataNotIsolated();
      return core.invoke<void>("bd_fs_copy", {
        src,
        dest,
        permanent,
    ...
    clear: async () => {
      ensureDataNotIsolated();
      if (permanent) {
        return core.invoke<void>("bd_fs_clear_data");
      }
      return core.invoke<void>("bd_fs_clear_cache");
    },
  ...
  return {
    ...
    trash: {
      ...
      listContent: async (rel: string = "") =>
        core.invoke<FsEntry[]>("bd_fs_trash_list_dir", { rel }),
      stat: async (rel: string = "") =>
        core.invoke<FsEntry>("bd_fs_trash_stat", { rel }),
      exists: async (rel: string = "") =>
        core.invoke<boolean>("bd_fs_trash_exists", { rel }),
      readText: async (rel: string) =>
        core.invoke<string>("bd_fs_trash_read_text", { rel }),
      readBytes: async (rel: string) =>
        core.invoke<string>("bd_fs_trash_read_bytes", { rel }),
    },
    diagnostics: {
      clear: async () => core.invoke<void>("bd_fs_diagnostics_clear"),
      remove: async (rel: string, recursive = false) =>
        core.invoke<void>("bd_fs_diagnostics_rm", { rel, recursive }),
      listContent: async (rel: string = "") =>
        core.invoke<FsEntry[]>("bd_fs_diagnostics_list_dir", { rel }),
      stat: async (rel: string = "") =>
        core.invoke<FsEntry>("bd_fs_diagnostics_stat", { rel }),
      exists: async (rel: string = "") =>
        core.invoke<boolean>("bd_fs_diagnostics_exists", { rel }),
      readText: async (rel: string) =>
        core.invoke<string>("bd_fs_diagnostics_read_text", { rel }),
      readBytes: async (rel: string) =>
        core.invoke<string>("bd_fs_diagnostics_read_bytes", { rel }),
    },
  };
}
