...
function scope(core: { invoke: BubbledeskAPI["invoke"] }, permanent: boolean): FsScopeMethods {
  return {
    listDir: (rel: string = "") => core.invoke<FsEntry[]>("bd_fs_list_dir", { rel, permanent }),
    mkdir: (rel: string) => core.invoke<void>("bd_fs_mkdir", { rel, permanent }),
    rm: (rel: string, recursive = false) => core.invoke<void>("bd_fs_rm", { rel, recursive, permanent }),
...
export function buildFs(core: { invoke: BubbledeskAPI["invoke"] }) {
  ...
  return {
    cache: { ...cache, clear: () => core.invoke<void>("bd_fs_clear_cache") },
    data,
    paths: () => core.invoke<FsPaths>("bd_fs_paths"),
